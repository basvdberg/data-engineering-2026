...
def build_design_patterns_site_map(repo_root: Path, from_file: Path) -> list[str]:
    """Cross-file navigation tree used in the design-patterns repository readme."""
...
def build_toc_block(
    ...
    *,
    ...
    lines: list[str] = []
    if (repo_root / "design-patterns").is_dir() and path.resolve() == (repo_root / "readme.md").resolve():
        lines.extend(build_design_patterns_site_map(repo_root, path))
        if headings:
            lines.append("")
    toc_headings = [
        (level, title, anchor)
...
def _collect_folder_tree_rows(
    root: Path,
    descriptions: dict[str, str],
    prefix: str = "",
    rel_parent: str = "",
) -> list[tuple[str, str | None]]:
    """Collect tree rows as (line_prefix, optional description). Directories only."""
    try:
        directories = sorted(
        ...
        return [(f"{prefix}[inaccessible]/", None)]
    ...
    for index, path in enumerate(directories):
        ...
        rel = f"{rel_parent}/{path.name}" if rel_parent else path.name
        rel_key = rel.replace("\\", "/")
        line = f"{prefix}{connector}{path.name}/"
        rows.append((line, descriptions.get(rel_key)))
        rows.extend(
            _collect_folder_tree_rows(path, descriptions, prefix + branch, rel_key)
        )
    return rows


def _format_folder_tree(repo_name: str, rows: list[tuple[str, str | None]]) -> list[str]:
    """Align optional descriptions in a second column after folder names."""
    lines = [repo_name + "/"]
    if not rows:
        return lines
    folder_lines = [line for line, _ in rows]
    max_width = max(len(line) for line in folder_lines)
    gap = 2
    for line, description in rows:
        if description:
            pad = max(gap, max_width - len(line) + gap)
            lines.append(f"{line}{' ' * pad}{description}")
        else:
            lines.append(line)
    return lines


def build_folder_tree_lines(repo_root: Path, descriptions: dict[str, str]) -> list[str]:
    """Build an ASCII tree of directories only (no files), with optional descriptions."""
    rows = _collect_folder_tree_rows(repo_root, descriptions)
    return _format_folder_tree(repo_root.name, rows)


def build_project_structure(repo_root: Path) -> str:
    descriptions = load_folder_descriptions(repo_root)
    tree_lines = build_folder_tree_lines(repo_root, descriptions)
    tree = "\n".join(tree_lines)
    return (
        f"{STRUCT_START}\n"
        "```text\n"
...
def update_file(
    ...
    *,
    ...
    structure = build_project_structure(repo_root)
...
def main(argv: list[str] | None = None) -> int:
    ...
    mode.add_argument(
        "--structure-only",
        action="store_true",
        help="Refresh only the project structure block (folders only).",
    )
